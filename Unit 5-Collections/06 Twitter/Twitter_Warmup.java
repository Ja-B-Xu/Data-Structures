...
class TJTwitter2 
{
   ...
   {
      //your code goes here      
      ListIterator<TJ_Status2> statusLI=statuses.listIterator();
      while(statusLI.hasNext()){
         String s=statusLI.next().getText();
   ...
   {  
      ...
      try{
         ...
         //String[] sArray=new String[91];
         ArrayList<String> sArray= new ArrayList<String>();
         int x=0;
         while(scan.hasNextLine()){
            sArray.add(x,scan.nextLine());
            x++;
         }
         ...
         System.out.println("common words file not found");
      }
   ...
   {
      //your code goes here  
      Comparable[] c = new Comparable[terms.size()];  
      for(int x=0;x<terms.size();x++){
         c[x]=(Comparable)terms.get(x);
      }
      sort(c);
      for(int y=0;y<terms.size();y++){
         terms.set(y,(String)c[y]);
      }
   ...
   private static void sort(Comparable[] array)
   {
      ...
      for(int x=0;x<len;x++){
         ...
         for(int y=0;y<(len-x);y++){
            if(array[y].compareTo(max)>0){
               max=array[y];
               maxPos=y;
            }
         }
         Comparable temp=array[maxPos];
         array[maxPos]=array[len-x-1];
         array[len-x-1]=temp;
      }
   }
