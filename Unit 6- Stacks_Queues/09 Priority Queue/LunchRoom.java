...
public class LunchRoom
{
   ...
   public static void main(String[] args)
   {
      ...
      for(time=0;time<=TIME;time++){
         if(Math.random()<0.2){
            ...
            if(gr<0.25){
               queue.add(new Customer(time,wait,9));
               totalFr++;
            }
            else if(gr<0.5){
               queue.add(new Customer(time,wait,10));
               totalSo++;
            }
            else if(gr<0.75){
               queue.add(new Customer(time,wait,11));
               totalJu++;
            }
            else{
               queue.add(new Customer(time,wait,12));
               totalSe++;
            }
            total++;
            currentLength++;
         ...
         if(!queue.isEmpty()){
            if(queue.peek().getWait()==1){
               queue.peek().waitOne();
               int wTime=time-queue.peek().getArrive();
               if(queue.peek().getGrade()==9){
                  if(wTime>maxWaitFr)
                     maxWaitFr=wTime;
                  totalFrWait+=wTime;
               }
               if(queue.peek().getGrade()==10){
                  if(wTime>maxWaitSo)
                     maxWaitSo=wTime;
                  totalSoWait+=wTime;
               }
               if(queue.peek().getGrade()==11){
                  if(wTime>maxWaitJu)
                     maxWaitJu=wTime;
                  totalJuWait+=wTime;
               }
               if(queue.peek().getGrade()==12){
                  if(wTime>maxWaitJu)
                     maxWaitSe=wTime;
                  totalSeWait+=wTime;
               }
      ...
      /* report the data  */
      System.out.println("Senior\t\t"+totalSe+"\t\t\t"+maxWaitSe+"\t\t\t\t"+(double)totalSeWait/totalSe);
      System.out.println("Junior\t\t"+totalJu+"\t\t\t"+maxWaitJu+"\t\t\t\t"+(double)totalJuWait/totalJu);
      System.out.println("Sophomore\t"+totalSo+"\t\t\t"+maxWaitSo+"\t\t\t\t"+(double)totalSoWait/totalSo);
      System.out.println("Freshman\t\t"+totalFr+"\t\t\t"+maxWaitFr+"\t\t\t\t"+(double)totalFrWait/totalFr);
   }
   ...
   public static void display(int t, PriorityQueue<Customer> q)
   {
      ...
      while(!q.isEmpty()){
         ...
         if(c.getGrade()==9)
            s+=c.getArrive()+":Fr, ";
         else if(c.getGrade()==10)
            s+=c.getArrive()+":So, ";
         else if(c.getGrade()==11)
            s+=c.getArrive()+":Ju, ";
         else
            s+=c.getArrive()+":Se, ";
      }
...
class Customer implements Comparable<Customer>
{
   ...
   private int grade;
   public Customer(int i, int w, int g){
      arrival=i;
      waitTime=w;
      grade=g;
   }
